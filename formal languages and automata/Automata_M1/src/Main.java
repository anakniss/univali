    ...
    private void analisarEntrada() {
        ...
        for (int i = 0; i < nSentencas; i++) {
            ...
            if (s.equals("+") || s.equals("-") || s.equals("*") || s.equals("/")) {
                resultado.append("operador aritmética: ").append(s).append("\n");
            } else if (reconheceSentenca(tabela, ef, s)) {
                resultado.append("sentença válida: ").append(s).append("\n");
            } else if (s.charAt(0) != 'a' && s.charAt(0) != 'b' &&
                    s.charAt(0) != 'c' && s.charAt(0) != 'd' && s.charAt(0) != 'e') {
                resultado.append("ERRO: símbolo(s) inválido(s): '").append(s).append("'\n");
            } else {
                resultado.append("ERRO: sentença inválida: '").append(s).append("'\n");
            }
        }
